...
class LTML(object):
    ...
    def get_sentences(self, pid, sid, encoding=None):
    ...
    # filter some pos words, now only support noun pos
    def get_words_by_pos(self, pid, sid, flags=['n','nh','ni','ns','nz','ws','j'],encoding=None):
       ret = [e.get('cont') for e in self.dom[1][pid][sid].findall('word') if e.get('pos') in flags]
       if encoding is None:
           return ret
       else:
           return [r.encode(encoding) for r in ret]
        ...
        # filter words by ner.
    def get_words_by_ner(self, pid, sid, flags=['S-Nh', 'S-Ni', 'S-Ns'],encoding=None):
       ret = [e.get('cont') for e in self.dom[1][pid][sid].findall('word') if e.get('ne') in flags]
       if encoding is None:
           return ret
       else:
           return [r.encode(encoding) for r in ret]
